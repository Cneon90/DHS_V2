...
void thClientConnect::setUser(const char* _user, const char* _pass){
	setUsermtx.lock(); 
	strncpy(ConnectHandler->pxConnectionAttr->cUser, _user, SERVER_CONN_ATTR_TEXT_MAX_SIZE - 1);
	ConnectHandler->pxConnectionAttr->cUser[SERVER_CONN_ATTR_TEXT_MAX_SIZE - 1] = '\0'; 
	strncpy(ConnectHandler->pxConnectionAttr->cPass, _pass, SERVER_CONN_ATTR_TEXT_MAX_SIZE - 1);
	ConnectHandler->pxConnectionAttr->cPass[SERVER_CONN_ATTR_TEXT_MAX_SIZE - 1] = '\0'; 
	setUsermtx.unlock();
}
