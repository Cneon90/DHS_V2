/**
	...
	класс осуществл€ет подключение к серверу, в качестве параметра 
	принимает реквизиты дл€ подключени€ (хост, порт, логин, пароль) 
	и класс команду ( setCommand(AbstractCommand* _cmd) )  
	
	¬ыполн€ет команду, и возвращает ответ. 
...
//			FCmd = nullptr;
		...
		void startThread() { 
		    if (threadClientConnect.joinable()) {
            	threadClientConnect.join();  // ƒожидаемс€ завершени€ потока
        	}
